...
const EnitexaPageShell = ({
    ...
    const toolbarRow = (
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-4">
            <button
                type="button"
                onClick={onBack}
                className="group pf-mono inline-flex w-fit items-center gap-2 rounded-full border border-white/15 px-4 py-2 text-[10px] font-medium uppercase tracking-[0.22em] text-slate-300 transition-all duration-300 hover:border-indigo-300/50 hover:text-white"
            >
    ...
    const topPad = shellHeader === 'toolbar' ? toolbarTop : 'pt-24 sm:pt-28';
    const chromeSurface = variant === 'saas-dark' ? 'enitexa-saas-shell-dark' : 'bg-[#070B17]';
    const bottomPad = shellHeader === 'toolbar' ? 'pb-20 lg:pb-24' : 'pb-20 sm:pb-24 lg:pb-28';
        ...
        <div className={`relative min-h-screen overflow-x-hidden ${chromeSurface} ${bottomPad} ${topPad} text-white`}>
            ...
            <div className="relative z-10 mx-auto w-full min-w-0 max-w-7xl px-4 sm:px-6 lg:px-8">
                {shellHeader === 'toolbar' ? (
                    ...
                    <div className="pb-4 pt-4 md:pt-10">
                        <div className="mb-8 sm:mb-10">{toolbarRow}</div>

                        <div className="pf-mono mb-5 flex items-center gap-3 text-[10px] uppercase tracking-[0.28em] text-white/35 sm:mb-6 sm:gap-4 sm:text-[11px] sm:tracking-[0.35em]">
                            {badge ? <span className="shrink-0 text-indigo-300">({badge})</span> : null}
                            <span className="h-px flex-1 bg-white/[0.08]" />
                        </div>

                        <h1 className="pf-display max-w-4xl text-[clamp(1.6rem,6.5vw,3.4rem)] font-bold leading-[1.05] text-white">
                            {title}
                        </h1>
                        {subtitle ? (
                            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-slate-400 sm:mt-6 sm:text-lg">
                                {subtitle}
                            </p>
                        ) : null}
                ...
                <div className={shellHeader === 'standard' ? 'mt-8 min-w-0 sm:mt-10' : 'min-w-0'}>
                    {children}
                </div>
            </div>
        </div>
