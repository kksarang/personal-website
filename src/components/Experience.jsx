        ...
        <section id="experience" className="relative border-t border-[#111110]/10 py-24 dark:border-white/10 sm:py-28">
            <div className="mx-auto max-w-7xl px-5 sm:px-8">
                <p className="port-mono mb-4 text-[11px] uppercase tracking-[0.35em] text-[#6b6b63] dark:text-white/40">
                    <span className="text-[#B8F03A]">(04)</span> Career
                </p>
                ...
                <div className="border-t border-[#111110]/10 dark:border-white/10">
                    {experiences.map((exp, index) => (
                        <article
                            ...
                            className="group grid gap-4 border-b border-[#111110]/10 py-10 dark:border-white/10 sm:grid-cols-[160px_1fr] sm:gap-10 lg:grid-cols-[200px_1fr]"
                        >
                            <p className="port-mono text-[11px] uppercase tracking-[0.18em] text-[#B8F03A]">
                                {exp.period}
                            </p>
                            <div>
                                <div className="flex items-start justify-between gap-4">
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm font-medium text-[#6b6b63] dark:text-white/50">{exp.company}</p>
                                    </div>
                                    <span className="port-mono hidden text-[10px] text-[#6b6b63] dark:text-white/30 sm:block">
                                        0{index + 1}
                                    </span>
                                </div>
                                <p className="mt-3 text-[#6b6b63] dark:text-white/50">{exp.description}</p>
                                {exp.highlights ? (
                                    <ul className="mt-5 space-y-2.5">
                                        {exp.highlights.map((highlight) => (
                                            <li key={highlight} className="flex gap-3 text-sm leading-relaxed text-[#6b6b63] dark:text-white/45">
                                                <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-[#B8F03A]" />
                                                {highlight}
                                            </li>
                                        ))}
                                    </ul>
